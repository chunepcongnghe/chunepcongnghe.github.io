...
def parse_post(path: Path):
    ...
    return text, m.group(1), m.end()


def is_valid_local(image: str, slug: str) -> tuple[bool, str]:
    expected = f"/assets/images/thumbs/{slug}.jpg"
    if image != expected:
        return False, ""
    p = ROOT / image.lstrip("/")
    if not p.exists():
...
def download(url: str) -> Image.Image | None:
    try:
        r = requests.get(url, headers={"User-Agent": UA, "Accept": "image/avif,image/webp,image/apng,image/*,*/*;q=0.8", "Referer": "https://www.google.com/"}, timeout=35, allow_redirects=True)
        r.raise_for_status()
...
def cover(im: Image.Image, size, xbias=0.5, ybias=0.5, zoom=1.0):
    ...
    left = int(max(0, nw - tw) * min(1, max(0, xbias)))
    top = int(max(0, nh - th) * min(1, max(0, ybias)))
...
def make_thumb(src: Image.Image, slug: str, category: str) -> Image.Image:
    ...
    if variant == 0:
        ...
        bg = ImageEnhance.Brightness(cover(src, SIZE, 1-xbias, ybias, 1.16).filter(ImageFilter.GaussianBlur(18))).enhance(0.72)
        fw, fh = 1040, 585
        fg = cover(src, (fw, fh), xbias, ybias, zoom)
        shadow = Image.new("RGBA", SIZE, (0,0,0,0))
        ImageDraw.Draw(shadow).rounded_rectangle((88, 57, 1132, 646), radius=36, fill=(0,0,0,82))
        ...
        base = cover(src, SIZE, xbias, ybias, zoom).convert("RGBA")
        detail = cover(src, (355, 675), 1-xbias, ybias, 1.24).convert("RGBA")
        base.alpha_composite(Image.new("RGBA", (10,675), (255,255,255,60)), (835,0))
        base.alpha_composite(detail, (845,0))
        canvas = base.convert("RGB")
    else:
        bg = ImageEnhance.Brightness(cover(src, SIZE, xbias, 1-ybias, 1.22).filter(ImageFilter.GaussianBlur(24))).enhance(0.62)
        fw, fh = 930, 560
        fg = cover(src, (fw, fh), 1-xbias, ybias, 1.03)
        canvas = bg.convert("RGBA")
        shadow = Image.new("RGBA", SIZE, (0,0,0,0))
        ImageDraw.Draw(shadow).rounded_rectangle((145, 70, 1095, 650), radius=42, fill=(0,0,0,95))
    ...
    accent = {"laptop": (0,174,180,130), "smartphone": (58,111,247,125), "camera": (240,145,35,125), "ai": (126,87,194,125)}.get(category.lower(), (0,153,158,115))
...
def main():
    ...
    for path in posts:
        ...
        valid, digest = is_valid_local(image, slug)
        if valid:
            manifest[slug] = {"post": str(path.relative_to(ROOT)), "image": image, "source": "existing-local", "sha256": digest, "size": list(SIZE)}
            print(f"KEEP {slug}")
            continue
        ...
        manifest[slug] = {"post": str(path.relative_to(ROOT)), "image": local, "source": used_source, "sha256": digest, "size": list(SIZE)}
    ...
    paths_seen, hashes_seen, errors = {}, {}, []
    for path in posts:
        _, front, _ = parse_post(path)
        if field(front, "published").lower() == "false":
            continue
        slug = field(front, "slug") or path.stem[11:]
        image = field(front, "image")
        valid, digest = is_valid_local(image, slug)
        if not valid:
            errors.append(f"{slug}: missing/broken/non-local thumbnail: {image}")
            continue
        if image in paths_seen:
            errors.append(f"{slug}: duplicate path with {paths_seen[image]}")
        paths_seen[image] = slug
        if digest in hashes_seen:
            errors.append(f"{slug}: duplicate binary with {hashes_seen[digest]}")
